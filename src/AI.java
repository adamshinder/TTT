...
public class AI
{
	...
	private State maxValue(State st)
	{
		...
		for (State child : generateSuccessors(st))
		{
			child.h = h(child) ; // would like to remeber what it's value is
			v = Math.max(v, minValue(child));
			if (bestState == null || child.h == v )
				bestState = child;
			//if we haven't picked one, pick the first one 
			//so we have something to compare later	
		}
		st.h = v; 
		return bestState; 
	...
	private State minValue(State st)
	{
		if (isTerminal(st)) return st; // "terminal" == game over, timed out, deep enough
		...
		{
			child.h = h(child) ; // would like to remeber what it's value is
			v = Math.max(v, maxValue(child).h); 
			//the state is not an integer but it has a heuristic so we need to add .h
			// v gets the worst choices and the thing you've seen so far
			if (bestState == null || child.h == v )
				bestState = child;
		}
